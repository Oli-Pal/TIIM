...
{
    public class GetPhotosForUserQueryHandler : IRequestHandler<GetPhotosForUserQuery, IEnumerable<PhotoToReturnResponse>>
    {
        ...
        public async Task<IEnumerable<PhotoToReturnResponse>> Handle(GetPhotosForUserQuery request, CancellationToken cancellationToken)
        {
            var photosFromRepo = await _photoRepo.GetPhotosForUserAsync(request.UserId);
